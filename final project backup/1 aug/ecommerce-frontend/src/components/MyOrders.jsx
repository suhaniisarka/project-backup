...
function MyOrders(){
    ...
    useEffect(()=>{
        axios.get("http://localhost:4000/api/orders?custid="+sessionStorage.getItem("id"))
        .then(resp=>{
                        ...
                        <tbody>
                            {details.map(x => (
                                <tr>
                                    ...
                                    <td><img className="mr-2 float-left" src={"http://localhost:4000/"+x.product.photo} width="100" />
                                    {x.product.pname}<br/>
